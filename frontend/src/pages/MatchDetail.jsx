...
export default function MatchDetail() {
  ...
  return (
    <div style={{ maxWidth: '1200px', margin: '0 auto', padding: '36px 24px' }}>
      ...
      {/* Layout principal : stats gauche + analyse droite */}
      <div style={{ display: 'grid', gridTemplateColumns: analyzed ? '1fr 1fr' : '1fr auto', gap: '20px', marginTop: '24px', alignItems: 'start' }}>
          ...
          {!analyzed ? (
            ...
            <>
              <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
                <button onClick={() => handleAnalyze(true)} style={{ fontFamily: 'var(--font-mono)', fontSize: '10px', letterSpacing: '0.12em', color: 'var(--text-muted)', background: 'transparent', border: '1px solid var(--border)', borderRadius: '3px', padding: '6px 14px', cursor: 'pointer' }}>
                  ↺ RAFRAÎCHIR
                </button>
              </div>
...
function MatchHeader({ match }) {
  ...
  return (
    <div style={{ background: 'var(--bg-surface)', border: '1px solid var(--border)', borderRadius: '4px', padding: '28px 32px', position: 'relative', overflow: 'hidden', marginTop: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', alignItems: 'center', gap: '20px', marginBottom: '20px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          {match.home_team?.crest_url && (
            <img src={match.home_team.crest_url} alt="" width={48} height={48} style={{ objectFit: 'contain' }} onError={(e) => { e.target.style.display = 'none' }} />
          )}
          <div>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(20px, 3.5vw, 36px)', letterSpacing: '0.03em', color: 'var(--text-primary)', lineHeight: 1, marginBottom: '4px' }}>{home}</div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px', justifyContent: 'flex-end' }}>
          <div style={{ textAlign: 'right' }}>
            <div style={{ fontFamily: 'var(--font-display)', fontSize: 'clamp(20px, 3.5vw, 36px)', letterSpacing: '0.03em', color: 'var(--text-primary)', lineHeight: 1, marginBottom: '4px' }}>{away}</div>
            <div style={{ fontFamily: 'var(--font-mono)', fontSize: '9px', letterSpacing: '0.16em', color: 'var(--text-muted)' }}>EXTÉRIEUR</div>
          </div>
          {match.away_team?.crest_url && (
            <img src={match.away_team.crest_url} alt="" width={48} height={48} style={{ objectFit: 'contain' }} onError={(e) => { e.target.style.display = 'none' }} />
          )}
        </div>
      </div>
        ...
        return (
          <div key={label} style={{ marginBottom: '12px' }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto 1fr', gap: '8px', alignItems: 'center', marginBottom: '4px' }}>
              ...
              <span style={{ fontFamily: 'var(--font-mono)', fontSize: '8px', color: 'var(--text-muted)', letterSpacing: '0.08em', textAlign: 'center', minWidth: '80px' }}>{label}</span>
...
function H2HCard({ h2h, homeStats, awayStats }) {
  ...
  const hPct  = Math.round((h2h.home_wins / total) * 100)
  const dPct  = Math.round((h2h.draws / total) * 100)
  const aPct  = Math.round((h2h.away_wins / total) * 100)
